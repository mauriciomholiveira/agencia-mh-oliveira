 ...
 * @return array
 */
function nerko_body_classes( $classes ) {
...
add_filter( 'body_class', 'nerko_body_classes' );

/**
 * Get tags.
 */
function nerko_get_tag() {
    $html = '';
    if ( has_tag() ) {
        $html .= '<div class="tg-post-tag"><h5 class="tag-title">' . esc_html__( 'Post Tags :', 'nerko' ) . '</h5>';
...
/** img alt-text **/
function nerko_img_alt_text( $img_er_id = null ) {
    $image_id = $img_er_id;
    ...
    if ( !empty( $image_id ) ) {
        ...
        $alt_text = esc_html__( 'Image Alt Text', 'nerko' );
    }

    return $alt_text;
}

// nerko_offer_sidebar_func
function nerko_offer_sidebar_func() {
    if ( is_active_sidebar( 'offer-sidebar' ) ) {

        dynamic_sidebar( 'offer-sidebar' );
    }
}
add_action( 'nerko_offer_sidebar', 'nerko_offer_sidebar_func', 20 );

// nerko_service_sidebar
function nerko_service_sidebar_func() {
    if ( is_active_sidebar( 'services-sidebar' ) ) {

        dynamic_sidebar( 'services-sidebar' );
    }
}
add_action( 'nerko_service_sidebar', 'nerko_service_sidebar_func', 20 );

// nerko_portfolio_sidebar
function nerko_portfolio_sidebar_func() {
    if ( is_active_sidebar( 'portfolio-sidebar' ) ) {

        dynamic_sidebar( 'portfolio-sidebar' );
    }
}
add_action( 'nerko_portfolio_sidebar', 'nerko_portfolio_sidebar_func', 20 );

// nerko_faq_sidebar
function nerko_faq_sidebar_func() {
    if ( is_active_sidebar( 'faq-sidebar' ) ) {

        dynamic_sidebar( 'faq-sidebar' );
    }
}
add_action( 'nerko_faq_sidebar', 'nerko_faq_sidebar_func', 20 );